...
Your task is to generate 3-5 UPDATED versions of the current chord progression that change its emotional character and mood while maintaining some musical continuity.

Current Key: ${currentKey}
Current Progression: ${currentProgressionText}
${compatibilityScore ? `Compatibility Score: ${compatibilityScore}%` : ''}
${compatibilityRationale ? `Analysis: ${compatibilityRationale}` : ''}
...
3. Each update should create a DISTINCTLY DIFFERENT emotional feeling from the original
...
Return ONLY a valid JSON array of 3-5 recommendations. Each recommendation must have:
...
Use proper chord symbols for ${currentKey} key. Include extensions when they enhance the emotional shift.`;
    ...
    const userMessage = `Generate 3-5 updated versions of the chord progression "${currentProgressionText}" in ${currentKey} that create different emotions and feelings while keeping some of the original progression intact.`;
    ...
    const validatedUpdates = updates.map((update, idx) => ({
      id: update.id || `update-${Date.now()}-${idx}`,
      ...
      isUpdate: true, // Flag to indicate this is an update recommendation
    }));
    ...
    console.error('Error generating progression updates:', error);
    return NextResponse.json(
